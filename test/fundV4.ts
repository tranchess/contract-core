...
describe("FundV4", function () {
    ...
    describe("Settlement of a non-empty fund", function () {
        ...
        const btcInFund = parseBtc("10");
        const protocolFeeQ = parseEther("10")
            .mul(DAILY_PROTOCOL_FEE_BPS)
            .div(10000 - DAILY_PROTOCOL_FEE_BPS);
        const equivalentTotalB = parseEther("5000").add(protocolFeeQ.mul(500));
        ...
        it("Should transfer fee to the fee collector", async function () {
            const oldBalance = await fund.trancheBalanceOf(TRANCHE_Q, feeCollector.address);
            const oldTotal = await fund.trancheTotalSupply(TRANCHE_Q);
            await advanceOneDayAndSettle();
            const newBalance = await fund.trancheBalanceOf(TRANCHE_Q, feeCollector.address);
            const newTotal = await fund.trancheTotalSupply(TRANCHE_Q);
            expect(newBalance).to.equal(oldBalance.add(protocolFeeQ));
            expect(newTotal).to.equal(oldTotal.add(protocolFeeQ));
        });
        ...
        it("Should not trigger upper rebalance when price is not high enough", async function () {
            ...
            const navSum = btcInFund.mul(BTC_TO_ETHER).mul(price).div(equivalentTotalB);
            const navR = navSum.sub(navB);
    ...
    describe("extrapolateNav()", function () {
        ...
        it("Should return the previous settlement if fund is empty", async function () {
            ...
            await primaryMarket.call(
                ...
                TRANCHE_Q,
                feeCollector.address,
                await fund.trancheBalanceOf(TRANCHE_Q, feeCollector.address),
                0
            );
            await primaryMarket.call(
                fund,
    ...
    describe("Rebalance", function () {
        ...
        describe("Balance refresh on interaction", function () {
            ...
            it("transfer()", async function () {
                ...
                await shareB.call(fund, "shareTransfer", addr1, addr2, 1);
    ...
    describe("Settlement with strategy", function () {
        ...
        it("reportProfit", async function () {
            await expect(fund.reportProfit(parseBtc("1"), parseBtc("0.1"), 1)).to.be.revertedWith(
                "Only strategy"
            );
            await expect(
                fund.connect(strategy).reportProfit(parseBtc("1"), parseBtc("2"), 1)
            ).to.be.revertedWith("Fee cannot exceed profit");
            await expect(
                fund.connect(strategy).reportProfit(parseBtc("1"), parseBtc("0.1"), parseBtc("0.2"))
            ).to.be.revertedWith("Fee cannot exceed profit");
            const totalFeeQ = parseBtc("0.1").mul(parseEther("10")).div(parseBtc("10.9"));
            const operatorFeeQ = parseBtc("0.02").mul(parseEther("10")).div(parseBtc("10.9"));
            await expect(
                fund
                    .connect(strategy)
                    .reportProfit(parseBtc("1"), parseBtc("0.1"), parseBtc("0.02"))
            )
                .to.emit(fund, "ProfitReported")
                .withArgs(parseBtc("1"), parseBtc("0.1"), totalFeeQ, operatorFeeQ);
            expect(await fund.getStrategyUnderlying()).to.equal(parseBtc("10"));
            expect(await fund.getTotalUnderlying()).to.equal(parseBtc("11"));
        ...
        it("Should reject strategy change with debt", async function () {
            ...
            await primaryMarket.call(fund, "primaryMarketAddDebtAndFee", parseBtc("0.1"), 0);
            await advanceBlockAtTime(startDay);
            await fund.settle();
        ...
        it("Should update NAV according to profit", async function () {
            ...
            await fund
                .connect(strategy)
                .reportProfit(parseBtc("1"), parseBtc("0.5"), parseBtc("0.1"));
